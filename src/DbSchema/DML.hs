...
type SubKey sch t p r = TSubRec (KeyNames sch t) (p,r)
type Key sch t p r = Tagged (KeyNames sch t) (SubKey sch t p r)
type ParConstr b sch t p r =
  ...
  , Ord (SubKey sch t p r)
  , Show p, Show r, Show (SubKey sch t p r)
...
class ( Db b, CRecDef b sch r
  ...
  dmlSelect :: (AppMon f m, CRecDef b sch q)
            => f (q,p) -> SessionMonad b m (f [r])
...
dmlSelectDef :: (DML b sch t p r, AppMon f m, CRecDef b sch q)
            => Proxy '(b,sch,t,r) -> f (q,p) -> SessionMonad b m (f [r])
dmlSelectDef (_::Proxy '(b,sch,t,r)) (ps ::f (q,p))
  ...
  where
    ...
    whereCond = T.intercalate " AND "
      $ zipWith (\n s -> format "{} = {}" (s, paramName @b n)) [0..]
                (map fst (recDbDef @b @sch @q) ++ dmlParNames @b @sch @t @p @r)
--
...
dmlUpdateDef (_ :: Proxy '(b,sch,t)) isDiff (vals :: f (p,[r],[r])) = do
  ...
  liftIO $ do
    putStrLn "ins': "
    mapM_ print ins'
    mapM_ print $ setInsUpd <$> vals' <*> ins' <*> us'
